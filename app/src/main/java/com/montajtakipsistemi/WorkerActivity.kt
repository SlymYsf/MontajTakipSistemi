...
import com.google.firebase.database.DataSnapshot // YENİ EKLENDİ
import com.google.firebase.database.DatabaseError // YENİ EKLENDİ
import com.google.firebase.database.FirebaseDatabase
import com.google.firebase.database.ValueEventListener // YENİ EKLENDİ
...
class WorkerActivity : AppCompatActivity(), LocationListener {
    ...
    private var isciIsmi = "Personel" // Sınıf seviyesine taşıdık ki her yerden ulaşalım
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        isciIsmi = intent.getStringExtra("ISCI_ISMI") ?: "Personel"
        tvHosgeldin.text = "Hoşgeldin, $isciIsmi"

        // --- YENİ EKLENEN KISIM: AÇILIŞTA SON DURUMU KONTROL ET ---
        sonDurumuGetir()
        // ---------------------------------------------------------
    ...
    // --- YENİ FONKSİYON: FİREBASE'DEN SON DURUMU ÇEKME ---
    private fun sonDurumuGetir() {
        ...
        // addListenerForSingleValueEvent sadece 1 kere çalışır (Sayfa açıldığında)
        dbRef.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    val sonDurum = snapshot.child("durum").value?.toString() ?: "Bekleniyor"
                    tvDurum.text = "Durum: $sonDurum"

                    // Eğer veritabanında "Çalışıyor" gözüküyorsa ama konum takibi durmuşsa (uyku modundan uyanma)
                    // arka planda konum takibini tekrar aktif edelim ki veriler kesilmesin.
                    if (sonDurum == "Çalışıyor" && !isTracking) {
                        if(konumIzniVarMi()){
                            konumTakibiniBaslatSessiz() // Toast mesaji göstermeyen versiyon
                        }
                    }
                }
            }
    ...
    // --- KONUM TAKİBİNİ SESSİZ BAŞLAT (Uyku modundan uyanınca) ---
    private fun konumTakibiniBaslatSessiz() {
